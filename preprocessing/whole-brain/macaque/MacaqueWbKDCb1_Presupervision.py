...
filepath='/wynton/group/ye/mtschmitz/macaquedevbrain/CAT_chang_kOut'#CAT_fixed_kallisto'
files=os.listdir(filepath)
files=[f for f in files if 'kOut' in f]
files=[f for f in files if '_kOut' != f]
fileList=files
#fileList=fileList[0:5]
print(fileList)
newfile='/wynton/group/ye/mtschmitz/macaquedevbrain/CAT_chang_h5ad/KDCbVelocityMacaqueWbPresupervise.h5ad'
min_genes=700
...
if True:#not os.path.exists(newfile):
    ...
    multiseq=sc.read('/wynton/group/ye/mtschmitz/macaquedevbrain/CAT_chang_h5ad/KDCbVelocityMultiseq.h5ad')
...
if True:#not os.path.exists(re.sub('\.h5ad','Processed.h5ad',newfile)):
    ...
    adata.obs.loc[:,['full_cellname','supervised_name','leiden']].to_csv('/wynton/group/ye/mtschmitz/macaquedevbrain/MIP_annotations/MacaqueGEsupervisednamesHippo.txt',index=None)
