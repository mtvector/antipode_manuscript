#usage: python MultiseqFuzzyWuzzyGeneral.py index_sample_key.txt fastq_R1_file.fq multiseq_index_seqs.txt cell_barcodes.txt out_dir flipped_indicator 
#assumes your fastqs are catt'd, reads are paired, gunzipped, and named with R1 and R2
#flipped_indicator == 1 if you used the multiseq index plate in the wrong orientation, else 0 or leave it empty
#usage example: python MultiseqFuzzyWuzzyGeneral.py $JOB_DIR/MultiseqSamples.txt $fqfile ~/code/macaque-dev-brain/multiseq/MultiseqIndices.txt $KOUT/outs/filtered_feature_bc_matrix/barcodes.tsv.gz $KOUT 0 
...
pf=sys.argv[6] if len(sys.argv) > 6 else '0'
